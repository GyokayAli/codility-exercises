...
namespace _07.FrogRiverOne
{
    class Program
    {
        static void Main(string[] args)
        {
            var result = FindFrogRiverOne(5, new int[] { 1, 3, 1, 4, 2, 3, 5, 4 });
        ...
        public static int FindFrogRiverOne(int X, int[] A)
        {
            ...
            for (int k = 0; k < A.Length; k++)
            {
                if (A[k] <= X)
                {
                    currentLeaves.Add(A[k]);
                }

                if (currentLeaves.Count == X)
                    return k;
            }
